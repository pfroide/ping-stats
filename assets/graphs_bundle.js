...
function openSheetFor(lic){
    ...
    // Modale joueur = récap complet (toutes phases), même si le filtre global est sur Phase 1 ou 2
    const phaseSel0 = 'all';
    ...
    _sheetRaf = requestAnimationFrame(()=>{
      ...
      // La modale joueur affiche TOUJOURS toutes les phases, indépendamment du
      // filtre global $phase. C'est un récapitulatif complet du joueur, pas un
      // sous-ensemble filtré (ce qui prêterait à confusion : on cliquerait sur
      // un joueur ayant 30 matchs et on en verrait 6).
      const phaseSel = 'all';
  ...
  // Au chargement initial, lire le club + saison sélectionnés depuis localStorage
  // et utiliser le data_url adapté (cumul ou saison spécifique).
  (function _initClubFromStorage(){
    try{
      const savedId = localStorage.getItem('ping_selected_club');
      const savedSeason = localStorage.getItem('ping_selected_season');
      const CUMUL_KEY = '__cumul__';

      // 1. Tenter d'abord l'élément `clubs-data-full` (avec saisons) — sinon
      //    fallback sur `clubs-registry` (compat v8 d'origine).
      let club = null;
      const fullEl = document.getElementById('clubs-data-full');
      if(fullEl){
        try{
          const clubs = JSON.parse(fullEl.textContent || '[]');
          const defClub = clubs.find(c => c.default) || clubs[0];
          club = (savedId && clubs.find(c => c.id === savedId)) || defClub;
        }catch(_){}
      }
      if(!club){
        const regEl = document.getElementById('clubs-registry');
        if(!regEl){
          // Pas de registry du tout : on ne fait rien (bundle s'occupera du fallback)
          if(!savedId) return;
        } else {
          const clubs = JSON.parse(regEl.textContent || '[]');
          club = clubs.find(c => c.id === savedId) || clubs.find(c => c.default) || clubs[0];
        }
      }
      if(!club) return;

      // 2. Choisir l'URL : saison si dispo et différente du cumul, sinon data_url
      let url = club.data_url;
      if(savedSeason && savedSeason !== CUMUL_KEY){
        const sd = club.season_data_urls || {};
        if(sd[savedSeason]) url = sd[savedSeason];
      }

      if(url){
        window._pendingClubDataUrl = url;
      }
    }catch(_){}
  })();
